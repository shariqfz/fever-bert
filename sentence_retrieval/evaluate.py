...
def main(evidence_file, golden_file):
    ...
    _, _, mrr, recall_at_k, Micro_f1 = fever_score_micro_f1(predictions, actual)

    tab = PrettyTable()
    tab.field_names = ["Retrieval MRR","Retrieval Micro F1" ,"Retrieval Macro F1", "Retrieval Recall@1", "Retrieval Recall@10",
                       "Retrieval Recall@100","Recall" , "OFEVER Score", "Retrieval Precision",]
    tab.add_row((round(mrr, 4), round(Micro_f1, 4), round(Macro_f1, 4), round(recall_at_k[1], 4), round(recall_at_k[10],4),
