...
function document_retrieval() {
  ...
  for filetype in {dev,train}; do
    ...
    if [ ! -f "$doc_ret_file" ]; then
      ...
      python3 'document-retrieval/run.py' \
          --db-file "$db_file" \
...
function sentence_retrieval() {
  ...
  if [ ! -f "$model_path/config.json" ]; then
    ...
    if [ ! -f "$tuning_file" ]; then
      ...
      python3 'sentence-retrieval/generate.py' \
          --db-file "$db_file" \
    ...
    python3 'sentence-retrieval/model.py' \
  ...
  for filetype in {dev,test,train}; do
    ...
    if [ ! -f "$sent_ret_file" ]; then
      ...
      python3 'sentence-retrieval/run.py' \
  ...
  python3 'sentence-retrieval/evaluate.py' \
...
function generate_submission() {
  ...
  if [ ! -f "$sub_path" ]; then
    ...
    for filetype in {dev,test,train}; do
      ...
      if [ ! -f "$sub_file" ]; then
        ...
        python3 'generate-submission/run.py' \
  ...
  python3 'generate-submission/evaluate.py' \
